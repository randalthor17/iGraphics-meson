...
typedef struct {
  ...
  float rotationCenterX, rotationCenterY; // Center of rotation relative to the
...
void timerCallback(int index) {
  if (!iAnimPause[index] && iAnimFunction[index]) {
    ...
    // since start int deltaTime = (currentTime - iAnimLastCallTime[index]); //
    // in seconds iAnimLastCallTime[index] = currentTime;
    iAnimFunction[index]();
  }
...
void iShowTexture(int x, int y, const char *filename, int width = -1,
...
void iShowImage2(int x, int y, Image *img) {
  ...
  if (x >= 0 && y >= 0) {
    glRasterPos2i(x, y);
    glDrawPixels(imgWidth, imgHeight, (channels == 4) ? GL_RGBA : GL_RGB,
  ...
  unsigned char *clippedData =
      new unsigned char[drawWidth * drawHeight * channels];
  ...
  glDrawPixels(drawWidth, drawHeight, (channels == 4) ? GL_RGBA : GL_RGB,
               GL_UNSIGNED_BYTE, clippedData);
  delete[] clippedData;
}

void iShowLoadedTexture(int x, int y, Image *img, int width = -1,
                        int height = -1, MirrorState mirror = NO_MIRROR) {
  iShowTexture2(x, y, img, width, height, mirror);
}

void iShowLoadedImage(int x, int y, Image *img, int width = -1, int height = -1,
                      MirrorState mirror = NO_MIRROR) {
  iShowTexture2(x, y, img, width, height, mirror);
}

void iShowImage(int x, int y, const char *filename, int width = -1,
                int height = -1, MirrorState mirror = NO_MIRROR,
                int ignoreColor = -1) {
  Image img;
  if (!iLoadImage(&img, filename, ignoreColor)) {
...
void iResizeImage(Image *img, int width, int height) {
  ...
  stbir_resize_uint8(data, imgWidth, imgHeight, 0, resizedData, width, height,
                     0, channels);
  stbi_image_free(data);
...
void iScaleImage(Image *img, double scale) {
  ...
  stbir_resize_uint8(data, img->width, img->height, 0, resizedData, newWidth,
                     newHeight, 0, channels);
...
bool isSpecialKeyPressed(int key) { return specialKeys[key]; }
