...
enum MirrorState { HORIZONTAL, VERTICAL };
...
void iFreeImage(Image *img) { stbi_image_free(img->data); }
...
void iShowImage2(int x, int y, Image *img, int ignoreColor) {
  ...
  for (int y = 0; y < drawHeight; y++) {
    for (int x = 0; x < drawWidth; x++) {
      ...
      bool ignore =
          (ignoreColor != -1 && r == (ignoreColor >> 16 & 0xFF) &&
           g == ((ignoreColor >> 8) & 0xFF) && b == ((ignoreColor) & 0xFF));
...
void iWrapImage(Image *img, int dx) {
  // Circular shift the image horizontally by dx pixels (positive = right,
  // negative = left)
  int width = img->width;
