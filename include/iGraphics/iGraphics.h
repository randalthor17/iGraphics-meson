 ...
 * This library is designed to be easy to use for beginners and supports basic
 * graphics operations. It includes features like image loading, sprite
...
typedef struct {
  ...
  float rotationCenterX = 0.0f,
        rotationCenterY =
            0.0f; // Center of rotation relative to the sprite's top-left corner
} Sprite;

enum MirrorState { NO_MIRROR, HORIZONTAL, VERTICAL, MIRROR_BOTH };
...
void iKeyboard(unsigned char, int)
    __attribute__((weak)); // Deprecated: Use iKeyPress instead
...
void iMouseDrag(int, int)
    __attribute__((weak)); // Renamed from iMouseMove to iMouseDrag
...
void iUpdateTexture(Image *img, bool resized = false) {
  ...
  if (resized) {
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, img->width, img->height, format,
                    GL_UNSIGNED_BYTE, img->data);
  }
}
...
void iIgnorePixels(Image *img, int ignoreColor = -1) {
  ...
  for (int i = 0; i < img->width * img->height * img->channels;
       i += img->channels) {
    if (img->data[i] == ignoreR && img->data[i + 1] == ignoreG &&
        img->data[i + 2] == ignoreB) {
      if (img->channels == 4) {
...
void iShowTexture2(int x, int y, Image *img, int width = -1, int height = -1,
                   MirrorState mirror = NO_MIRROR) {
  int imgWidth = width == -1 ? img->width : width;
  int imgHeight = height == -1 ? img->height : height;

  if (x + imgWidth <= 0 || y + imgHeight <= 0 || x >= iScreenWidth ||
      y >= iScreenHeight)
    return;
  if (img->textureId == 0) {
    if (!iLoadTexture(img)) {
  ...
  // iRectangle(x, y, imgWidth, imgHeight); // Uncomment for debugging rectangle
  // bounds
...
//     unsigned char *clippedData = new unsigned char[drawWidth * drawHeight *
//     channels]; int srcStride = imgWidth * channels; int dstStride = drawWidth
//     * channels;
...
//     glDrawPixels(drawWidth, drawHeight, (channels == 4) ? GL_RGBA : GL_RGB,
//     GL_UNSIGNED_BYTE, clippedData); delete[] clippedData;
// }

void iShowLoadedImage2(int x, int y, Image *img, int width = -1,
                       int height = -1, MirrorState mirror = NO_MIRROR) {
...
void iShowSVG2(double x, double y, const char *filepath, double scale = 1.0,
               MirrorState mirror = NO_MIRROR) {
  // Load SVG
  Image img;
...
void iShowLoadedSVG2(double x, double y, Image *img,
                     MirrorState mirror = NO_MIRROR) {
...
void iResizeImage(Image *img, int width, int height) {
  ...
  stbir_resize_uint8_srgb(data, imgWidth, imgHeight, 0, resizedData, width,
                          height, 0, layout);
  // stbir_resize_uint8(data, imgWidth, imgHeight, 0, resizedData, width,
  // height, 0, channels);
  stbi_image_free(data);
...
void iScaleImage(Image *img, double scale) {
  ...
  stbir_resize_uint8_srgb(data, img->width, img->height, 0, resizedData,
...
int iCheckImageSpriteCollision(int x1, int y1, Image *img, Sprite *s) {
  if (!img || !s || !s->frames || s->currentFrame < 0 ||
  ...
  for (int y = overlapMinY; y < overlapMaxY; y++) {
    for (int x = overlapMinX; x < overlapMaxX; x++) {
      ...
      if (localX1 < 0 || localY1 < 0 || localX1 >= img->width ||
          localY1 >= img->height || localX2 < 0 || localY2 < 0 ||
          localX2 >= frame->width || localY2 >= frame->height)
        continue;

      unsigned char *pixel1 =
          &img->data[(localY1 * img->width + localX1) * img->channels];
      unsigned char *pixel2 =
          &frame->data[(localY2 * frame->width + localX2) * frame->channels];
...
int iCheckImageCollision(int x1, int y1, Image *img1, int x2, int y2,
                         Image *img2) {
  ...
  for (int y = overlapMinY; y < overlapMaxY; y++) {
    for (int x = overlapMinX; x < overlapMaxX; x++) {
      ...
      unsigned char *pixel1 =
          &img1->data[(localY1 * w1 + localX1) * img1->channels];
      unsigned char *pixel2 =
          &img2->data[(localY2 * w2 + localX2) * img2->channels];
...
int iCheckCollision(Sprite *s1, Sprite *s2) {
  ...
  if (!s1 || !s2 || !s1->frames || !s2->frames || !s1->collisionMask ||
      !s2->collisionMask)
    return 0;
  ...
  auto computeRotatedAABB = [](float x, float y, int w, int h, float pivotX,
                               float pivotY, float cosT, float sinT) {
  ...
  std::tie(minX1, maxX1, minY1, maxY1) =
      computeRotatedAABB(s1->x, s1->y, w1, h1, s1->rotationCenterX,
                         s1->rotationCenterY, cos1, sin1);

  float minX2, maxX2, minY2, maxY2;
  std::tie(minX2, maxX2, minY2, maxY2) =
      computeRotatedAABB(s2->x, s2->y, w2, h2, s2->rotationCenterX,
                         s2->rotationCenterY, cos2, sin2);
  ...
  for (int y = overlapMinY; y <= overlapMaxY; y++) {
    for (int x = overlapMinX; x <= overlapMaxX; x++) {
      ...
      float localX1 =
          invCos1 * dx1 - invSin1 * dy1 + (s1->rotationCenterX - s1->x);
      float localY1 =
          invSin1 * dx1 + invCos1 * dy1 + (s1->rotationCenterY - s1->y);
      ...
      float localX2 =
          invCos2 * dx2 - invSin2 * dy2 + (s2->rotationCenterX - s2->x);
      float localY2 =
          invSin2 * dx2 + invCos2 * dy2 + (s2->rotationCenterY - s2->y);
      ...
      if (ix1 >= 0 && iy1 >= 0 && ix1 < w1 && iy1 < h1 && ix2 >= 0 &&
          iy2 >= 0 && ix2 < w2 && iy2 < h2) {
        int idx1 = iy1 * w1 + ix1;
        int idx2 = iy2 * w2 + ix2;
        if (s1->collisionMask[idx1] && s2->collisionMask[idx2]) {
          ...
          // If you want to return immediately on first collision, uncomment the
          // next line return 1;
        }
      }
    }
...
void iLoadFramesFromSheet2(Image *frames, const char *filename, int rows,
                           int cols, int ignoreColor = -1) {
...
void iLoadFramesFromSheet(Image *frames, const char *filename, int rows,
                          int cols) {
...
void iLoadFramesFromFolder2(Image *frames, const char *folderPath,
                            int ignoreColor = -1) {
...
void iUnScale() { glPopMatrix(); }

void iShowSprite(const Sprite *s) {
  if (!s || !s->frames) {
    return;
  }
  iRotate(s->rotationCenterX, s->rotationCenterY, s->rotation);
...
void iSetLineWidth(float width = 1.0) { glLineWidth(width); }
...
void iText(double x, double y, const char *str,
...
void iTextBold(double x, double y, const char *str,
               void *font = GLUT_BITMAP_8_BY_13) {
...
void iTextAdvanced(double x, double y, const char *str, float scale = 0.3,
                   float weight = 1.0, void *font = GLUT_STROKE_ROMAN) {
...
int isKeyPressed(unsigned char key) { return keys[key]; }
...
int isSpecialKeyPressed(int key) { return specialKeys[key]; }
...
void reshapeFF(int width, int height) {
  ...
  // glutReshapeWindow(iSmallScreenWidth, iSmallScreenHeight); // Comment above
  // lines and uncomment this line to disable window resizing. (Credit: Mohammad
  // Kamrul Hasan)
}

void iHideCursor() { glutSetCursor(GLUT_CURSOR_NONE); }

void iShowCursor() { glutSetCursor(GLUT_CURSOR_INHERIT); }
...
void iWindowedMode(int width = 500, int height = 500,
...
void iStartMainLoop() { glutMainLoop(); }
void iOpenWindow(int width = 500, int height = 500,
                 const char *title = "iGraphics", int fullscreen = 0) {
  // Ensure GLUT was initialized
  iInitGlut();
  ...
  if (fullscreen) {
    ...
    glutGameModeString(
        gameModeStr); // Supports: 640×480 800×600 1024×768 1280×720 1366x768
    if (glutGameModeGet(GLUT_GAME_MODE_POSSIBLE)) {
      isFullScreen = 1;
      glutEnterGameMode();
    } else {
...
void iInitialize(int width = 500, int height = 500,
                 const char *title = "iGraphics") {
